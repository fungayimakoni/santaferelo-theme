	...
	if(get_field('form_assembly_id')):
		...
		echo '<section class="main-layout inner-page-layout">';
			echo '<div class="container-fluid default-container">';
											...
											echo '<div class="uk-width-5-6@l">';
												...
												if($type || $v_source || $f_source):
													echo '<div class="cta">';
														...
														echo '<a href="'.$link.'" '.( ( $type == 'video' ) ? 'uk-toggle' : '').'>'.$red.'</a>';
															if( $v_source ):
																echo '<div id="modal-media-youtube" class="uk-flex-top" uk-modal>';
																	echo '<div class="uk-modal-dialog uk-width-auto uk-margin-auto-vertical">';
																        ...
																        if($v_source == 'youtube'):
																        	echo '<iframe src="https://www.youtube-nocookie.com/embed/'.$video.'" allowFullScreen="allowFullScreen" width="560" height="315" frameborder="0" uk-responsive uk-video></iframe>';
																        endif;
																        if($v_source == 'hosted'):
														...
														if(get_field('form_page_link2')):
															...
															echo '<a href="'.get_field('form_page_link2').'" style="margin-left: 25px;">'.$red.'</a>';
														endif;
														// if($type == 'zoom'):
														// 	$red = 'Register';
														// 	echo '<a href="'.$link.'" style="margin-left: 25px;">'.$red.'</a>';
														// endif;
													echo '</div>';
												endif;
											echo '</div>';
				...
				if( have_rows('content_blocks') ):
					echo '<div class="content-block">';
					    while ( have_rows('content_blocks') ) : the_row();
							...
							elseif( get_row_layout() == 'image_background_content_module' ):
					        	$group = get_sub_field('mobility_image');
								if($group):
									echo '<div class="container-fluid fourth-content gms-custom">';
										if($group['image']):
											echo '<div class="uk-cover-container">';
												echo '<img src="'.$group['image']['url'].'" alt="" uk-cover>';
												echo '<div class="uk-overlay uk-position-cover uk-overlay uk-light uk-flex uk-flex-center uk-flex-middle">';
													if($group['title']):
														echo '<div class="uk-position-center">';
															echo '<div>';
										...
										echo '<div class="uk-grid-collapse" uk-grid>';
											...
											if( $item['name'] || $item['title'] || $item['description'] ):
												echo '<div class="profile-meta uk-width-2-3@s">';
													if($item['name']):
														echo '<div class="name" style="font-weight: bold;">'.$item['name'].'</div>';
													endif;
													if ($item['title']):
														echo '<div class="title" style="color: #869097; font-weight: bold;">'.$item['title'].'</div>';
													endif;
													if( $item['description'] ):
				...
				if(is_single(array(8454,8485,8483))):
				else:
					...
					echo '<div class="row">';
						echo '<div class="share">';
							...
							echo '<div class="return">';
								...
								echo '<a href="'.get_post_type_archive_link($cpt).'"><span uk-icon="icon: chevron-left; ratio: 1.25"></span>Back to events</a>';
							echo '</div>';
						echo '</div>';
					echo '</div>';
				echo '</div>';
